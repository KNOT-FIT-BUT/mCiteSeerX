...
def sendUrlCiteSeerX_EXTENDED(keywordsPhrase, title_arg, author_name,
    ...
    if (type(Citation) is not bool):
        raise ValueError("Prohited value of Citation")
    if (type(Sort) is not int):
        raise ValueError("Prohited value of Sort")
    if (Sort < 0 or Sort > 4):
        raise ValueError("Prohited value of Sort")
    try:
        ...
        raise ValueError("Bad Value of entered argument")
    if (Sort > 4):
        raise ValueError("Prohibited value of Sort")
    if (min_cit is True):
        raise ValueError("Prohibited value of argument Min Citations")
    try:
